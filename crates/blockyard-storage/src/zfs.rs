...
impl StorageBackend for ZfsBackend {
    ...
    async fn pool_capacity(&self) -> blockyard_common::Result<(u64, u64)> {
        ...
        let total: u64 = parts[0]
            .parse()
            .map_err(|e| blockyard_common::Error::Storage(format!("parse capacity: {e}")))?;
        let used: u64 = parts[1]
            .parse()
            .map_err(|e| blockyard_common::Error::Storage(format!("parse used: {e}")))?;
...
impl ZfsBackend {
    ...
    pub async fn receive_zvol(&self, volume_id: &VolumeId) -> blockyard_common::Result<()> {
        let zvol_name = format!("{}/vol-{}", self.pool_name, volume_id);
