...
impl RebalanceEngine {
    ...
    pub fn compute_moves(
        ...
        for volume in &cluster.volumes {
            ...
            let ideal_set: std::collections::HashSet<NodeId> =
                ideal.iter().copied().collect();
    ...
    pub fn start_syncing(&self, volume_name: &str) -> bool {
        let mut active = self.active_moves.lock();
        if let Some(m) = active.iter_mut().find(|m| m.volume_name == volume_name && m.state == MoveState::Pending) {
    ...
    pub fn start_promoting(&self, volume_name: &str) -> bool {
        let mut active = self.active_moves.lock();
        if let Some(m) = active.iter_mut().find(|m| m.volume_name == volume_name && m.state == MoveState::Syncing) {
...
mod tests {
    ...
    fn test_detect_imbalance_balanced_cluster() {
        ...
        assert!(result.is_empty(), "evenly loaded cluster should have no imbalance");
    }
    ...
    fn test_detect_imbalance_low_threshold() {
        let engine = make_engine();
        let nodes = vec![
            make_node(1, gb(100), gb(55)),
            make_node(2, gb(100), gb(45)),
        ];
